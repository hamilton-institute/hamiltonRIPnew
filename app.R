...
server <- function(input, output) {
  ...
  observeEvent(input$view_shape_click, { # Plotting excess mortality plots for each region after clicking on the map
    ...
    output$plot2 <- renderPlotly({
      ...
      p <- merged_df %>% ggplot(aes(Date, value)) +
  ...
  output$plot3 <- renderPlotly({
    national_plot <- total %>% ggplot(aes(Date, value)) +
      geom_line(color = "#084C95") +
      ylab("Excess postings (%)") +
      geom_hline(yintercept = 0, linetype = "dotted") +
  ...
  output$plot4 <- renderPlotly({
    heat <- merged_df %>%
      select(Group, Month, Monthly_Notices, Ref_Level) %>%
      na.omit()
    heat_grouped <- heat %>%
    ...
    heatmap.plot <- ggplot(data = heat.long, aes(x = Month, y = Region, Excess = Excess)) +
      ...
      scale_fill_gradient2(limits = c(-100, 100), high = "darkred", low = "#084C95", oob = squish) +
